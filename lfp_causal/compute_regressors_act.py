...
from lfp_causal.qlearning_act import fit_qlearning
...
def bincumsum(x, prior=1.1):
    ...
    # roll and set the first element as the prior to obtain t-1
    x = np.roll(x, 1)
    x[0] = 0  # ensure first element is 1
...
if __name__ == '__main__':
    ...
    for file in os.listdir(csv_dir):
        ...
        if file.endswith('.csv'):
            ...
            fname_beh = op.join(beh_dir, '{0}_act.xlsx'.format(session))
